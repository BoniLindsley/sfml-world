...
struct Player {
  ...
  Player()
      : position(400, 300), color(sf::Color::Blue), socket(nullptr),
        isLoggedIn(false) {}
};
    ...
    while (running) {
      ...
      if (listener.accept(*client) == sf::Socket::Done) {
        ...
        std::thread clientThread(
            &MMORPGServer::handleClient, this, clients.back().get());
        clientThread.detach();
      }
    }
  }
...
private:
  ...
  void handleLogin(sf::TcpSocket* client, sf::Packet& packet) {
    ...
    if (it != players.end() && it->second.isLoggedIn) {
      ...
      response << LOGIN_FAILED
               << std::string("Username already logged in");
    ...
    for (auto& pair : players) {
      if (pair.second.socket == client && pair.second.isLoggedIn) {
        ...
        for (const auto& otherPair : players) {
          if (otherPair.second.isLoggedIn &&
              otherPair.second.socket != client) {
    ...
    for (auto& pair : players) {
      if (pair.second.socket == client) {
        if (pair.second.isLoggedIn) {
          ...
          std::cout << "Player disconnected: " << pair.first
                    << std::endl;
          broadcastPlayerUpdate(pair.first);
        }
        break;
  ...
  void sendPlayerList(sf::TcpSocket* client) {
    for (const auto& pair : players) {
      if (pair.second.isLoggedIn) {
        sf::Packet packet;
        packet << PLAYER_UPDATE << pair.first << pair.second.position.x
               << pair.second.position.y << pair.second.color.r
               << pair.second.color.g << pair.second.color.b
               << true; // isLoggedIn
        client->send(packet);
      }
    }
  ...
  void broadcastPlayerUpdate(const std::string& username) {
    ...
    sf::Packet broadcast;
    broadcast << PLAYER_UPDATE << username << player.position.x
              << player.position.y << player.color.r << player.color.g
              << player.color.b << player.isLoggedIn;
